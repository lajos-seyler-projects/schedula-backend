...
def test_group_permissions_list_GET(user, auth_drf_client):
    ...
    assert set(results[0].keys()) == {"id", "name", "codename", "content_type", "user_count"}


def test_group_permissions_POST_and_DELETE_requires_permission(user, user_drf_client):
    group = GroupFactory()
    target = PermissionFactory()
    response = user_drf_client.post(get_group_permissions(group.name), data={"permissions": [target.id]}, format="json")
    assert response.status_code == status.HTTP_403_FORBIDDEN

    response = user_drf_client.delete(
        get_group_permissions(group.name), data={"permissions": [target.id]}, format="json"
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
def test_group_permissions_list_POST(user, auth_drf_client):
    ...
    response = client.post(get_group_permissions(group.name), data={"permissions": [target.id]}, format="json")
...
def test_group_permissions_list_DELETE(user, auth_drf_client):
    ...
    client, _ = auth_drf_client("users.manage_group_permissions", user=user)
    response = client.delete(get_group_permissions(group.name), data={"permissions": [target.id]}, format="json")
    assert response.status_code == status.HTTP_204_NO_CONTENT
    assert group.permissions.count() == 1
